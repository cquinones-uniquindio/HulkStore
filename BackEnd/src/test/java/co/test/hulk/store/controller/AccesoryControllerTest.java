...
public class AccesoryControllerTest {
	...
	public void testGetAllAccesories() throws Exception {
		...
		Accesory accesory = new Accesory("Superman");
		array.add(accesory);
		Mockito.when(accesoryRepository.findAll()).thenReturn(array);
		mvc.perform(get("/api/v1/accesories").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andExpect(jsonPath("$[0].name", is(array.get(0).getName())));		
	}
